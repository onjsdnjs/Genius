...
import name.yalsooni.common.util.support.InstanceMap;
import name.yalsooni.common.util.support.Log;
import name.yalsooni.genius.definition.service.UtilService;
import name.yalsooni.genius.replace.errcode.StringReplaceErrCode;
import name.yalsooni.genius.replace.filter.StringReplaceFilter;
import name.yalsooni.genius.replace.vo.StringReplaceInfo;
import name.yalsooni.genius.repository.PropertyReader;
 ...
 */
public class StringReplace implements UtilService {
    ...
    public StringReplace(StringReplaceInfo defaultInfo) throws NullPointerException {
        if (defaultInfo == null) {
            throw new NullPointerException(StringReplaceErrCode.G10_0001);
        } else {
    ...
    public void initialize() throws Exception {
        ...
        if (excelFilePath == null) {
            throw new NullPointerException(StringReplaceErrCode.G10_0002);
        }
    ...
    private Map<String, String> templateFileToMap(File tempDirectory) throws OutOfMemoryError, Exception {
        ...
        if (!tempDirectory.isDirectory()) throw new Exception(StringReplaceErrCode.G10_0003 + tempDirectory);
    ...
    private String replace(String targetTemp, List<String> subject, List<String> values) throws Exception {
        ...
        for (int j = START_IDX; j < values.size(); j++) {
            ...
            try {
                ...
                throw new Exception(StringReplaceErrCode.G10_0004 + "COLUMN NUMBER : " + (j + 1) + ", KEY : " + key + ", VALUE : " + value, e);
            }
        }
